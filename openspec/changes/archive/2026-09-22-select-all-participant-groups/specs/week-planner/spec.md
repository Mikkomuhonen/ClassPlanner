...
The popup's participant selection SHALL provide a "Valitse kaikki ryhmät →" control, in addition to each group's own "Valitse kaikki →" control, that adds every member of every registry group to the slot's selected participants in a single action. After using either select-all control, the user MAY still remove any individual participant from the selection via its chip or by toggling its member button off.
...
#### Scenario: Select all groups at once
- **WHEN** the teacher activates the "Valitse kaikki ryhmät →" control in the popup's participant selection
- **THEN** every member of every registry group is added to the slot's selected participants

#### Scenario: Deselect individual participants after selecting all groups
- **WHEN** the teacher activates "Valitse kaikki ryhmät →" and then removes one participant via its chip or member button
- **THEN** that participant is no longer selected while all other previously added participants remain selected
